...
#define LL_MPU_ATTRIBUTES_NUMBER7          7U
	...
	__STATIC_INLINE void LL_MPU_ConfigRegion(uint32_t Region,
			uint32_t Attributes, uint32_t AttrIndx, uint32_t BaseAddress,
		...
		WRITE_REG(MPU->RLAR,
				((LimitAddress & MPU_RLAR_LIMIT_Msk) | AttrIndx | MPU_RLAR_EN_Msk));
	}
	...
	__STATIC_INLINE void LL_MPU_ConfigAttributes(uint32_t AttIndex,
		...
		/* When selected index is in range [4;7] */
			...
			MODIFY_REG(MPU->MAIR1, (0xFFUL << ((AttIndex - 4U) * 8U)),
