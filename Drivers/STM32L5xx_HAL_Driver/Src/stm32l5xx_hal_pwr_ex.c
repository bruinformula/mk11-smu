...
HAL_StatusTypeDef HAL_PWREx_ControlVoltageScaling(uint32_t VoltageScaling) {
	...
	wait_loop_index = ((PWR_VOSF_SETTING_DELAY_VALUE
			* (SystemCoreClock / 100000U)) / 10U) + 1U;
 ...
 * @retval HAL Status
 */
HAL_StatusTypeDef HAL_PWREx_ConfigSRAM2ContentRetention(
		uint32_t SRAM2ContentRetention) {
...
HAL_StatusTypeDef HAL_PWREx_DisableLowPowerRunMode(void) {
	...
	/* and at least one iteration loop */
	wait_loop_index = ((PWR_REGLP_SETTING_DELAY_VALUE
			* (SystemCoreClock / 100000U)) / 10U) + 1U;
...
HAL_StatusTypeDef HAL_PWREx_SMPS_SetMode(uint32_t OperatingMode) {
	...
	if (OperatingMode == PWR_SMPS_HIGH_POWER) {
		...
		if (HAL_PWREx_GetVoltageRange() != PWR_REGULATOR_VOLTAGE_SCALE2) {
			...
			if (READ_BIT(pwr_sr1, PWR_SR1_SMPSHPRDY | PWR_SR1_SMPSBYPRDY)
					== 0U) {
				/* Already in SMPS Low-power mode */
				/* Nothing to configure      */
			} else {
				/* Switch to Low-power mode */
				MODIFY_REG(PWR->CR4, (PWR_CR4_SMPSBYP | PWR_CR4_SMPSLPEN),
						PWR_CR4_SMPSLPEN);
			}
		}
	...
	/* and at least one iteration loop */
	wait_loop_index = ((PWR_MODE_CHANGE_DELAY_VALUE
			* (SystemCoreClock / 100000U)) / 10U) + 1U;

	while ((HAL_PWREx_SMPS_GetEffectiveMode() != OperatingMode)
			&& (wait_loop_index != 0U)) {
		wait_loop_index--;
	}
