...
__RAM_FUNC HAL_StatusTypeDef HAL_FLASHEx_OB_DBankConfig(uint32_t DBankConfig) {
  ...
  if ((reg & FLASH_SECWM1R1_SECWM1_PSTRT) > ((reg & FLASH_SECWM1R1_SECWM1_PEND) >> FLASH_SECWM1R1_SECWM1_PEND_Pos))
  {
	...
	reg = FLASH->WRP1AR;
	if ((reg & FLASH_WRP1AR_WRP1A_PSTRT)
			<= ((reg & FLASH_WRP1AR_WRP1A_PEND) >> FLASH_WRP1AR_WRP1A_PEND_Pos)) {
		MODIFY_REG(FLASH->WRP1AR,
				(FLASH_WRP1AR_WRP1A_PSTRT | FLASH_WRP1AR_WRP1A_PEND),
				FLASH_WRP1AR_WRP1A_PSTRT);
	}

	/* Disable WRP zone 1 of 2nd bank if needed */
	reg = FLASH->WRP1BR;
	if ((reg & FLASH_WRP1BR_WRP1B_PSTRT)
			<= ((reg & FLASH_WRP1BR_WRP1B_PEND) >> FLASH_WRP1BR_WRP1B_PEND_Pos)) {
		MODIFY_REG(FLASH->WRP1BR,
				(FLASH_WRP1BR_WRP1B_PSTRT | FLASH_WRP1BR_WRP1B_PEND),
				FLASH_WRP1BR_WRP1B_PSTRT);
	}

	/* Disable WRP zone 2 of 1st bank if needed */
	reg = FLASH->WRP2AR;
	if ((reg & FLASH_WRP2AR_WRP2A_PSTRT)
			<= ((reg & FLASH_WRP2AR_WRP2A_PEND) >> FLASH_WRP2AR_WRP2A_PEND_Pos)) {
		MODIFY_REG(FLASH->WRP2AR,
				(FLASH_WRP2AR_WRP2A_PSTRT | FLASH_WRP2AR_WRP2A_PEND),
				FLASH_WRP2AR_WRP2A_PSTRT);
	}

	/* Disable WRP zone 2 of 2nd bank if needed */
	reg = FLASH->WRP2BR;
	if ((reg & FLASH_WRP2BR_WRP2B_PSTRT)
			<= ((reg & FLASH_WRP2BR_WRP2B_PEND) >> FLASH_WRP2BR_WRP2B_PEND_Pos)) {
		MODIFY_REG(FLASH->WRP2BR,
				(FLASH_WRP2BR_WRP2B_PSTRT | FLASH_WRP2BR_WRP2B_PEND),
				FLASH_WRP2BR_WRP2B_PSTRT);
	}
