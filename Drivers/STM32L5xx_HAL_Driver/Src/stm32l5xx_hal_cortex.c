...
static void MPU_ConfigRegion(MPU_Type *MPUx,
	...
	assert_param(
			IS_MPU_REGION_PERMISSION_ATTRIBUTE(MPU_RegionInit->AccessPermission));
	...
	MPUx->RLAR = (((uint32_t) MPU_RegionInit->LimitAddress & 0xFFFFFFE0U)
			| ((uint32_t) MPU_RegionInit->AttributesIndex
...
static void MPU_ConfigMemoryAttributes(MPU_Type *MPUx,
	...
	*(mair) = attr_values
			| ((uint32_t) MPU_AttributesInit->Attributes << (attr_number * 8U));
}
