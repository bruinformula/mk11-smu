...
static void FDCAN_CopyMessageToRAM(const FDCAN_HandleTypeDef *hfdcan,
		const FDCAN_TxHeaderTypeDef *pTxHeader, const uint8_t *pTxData,
...
HAL_StatusTypeDef HAL_FDCAN_Init(FDCAN_HandleTypeDef *hfdcan) {
	...
	assert_param(
			IS_FDCAN_MAX_VALUE(hfdcan->Init.StdFiltersNbr, SRAMCAN_FLS_NBR));
	assert_param(
			IS_FDCAN_MAX_VALUE(hfdcan->Init.ExtFiltersNbr, SRAMCAN_FLE_NBR));
  ...
  if (hfdcan->State == HAL_FDCAN_STATE_RESET)
  {
	...
	hfdcan->Instance->NBTP = ((((uint32_t) hfdcan->Init.NominalSyncJumpWidth
	...
	if (hfdcan->Init.FrameFormat == FDCAN_FRAME_FD_BRS) {
		hfdcan->Instance->DBTP = ((((uint32_t) hfdcan->Init.DataSyncJumpWidth
...
HAL_StatusTypeDef HAL_FDCAN_ConfigFilter(FDCAN_HandleTypeDef *hfdcan,
	...
	if ((state == HAL_FDCAN_STATE_READY) || (state == HAL_FDCAN_STATE_BUSY)) {
		...
		if (sFilterConfig->IdType == FDCAN_STANDARD_ID) {
			/* Check function parameters */
			assert_param(
					IS_FDCAN_MAX_VALUE(sFilterConfig->FilterIndex, (hfdcan->Init.StdFiltersNbr - 1U)));
		...
		} else /* sFilterConfig->IdType == FDCAN_EXTENDED_ID */
		{
			/* Check function parameters */
			assert_param(
					IS_FDCAN_MAX_VALUE(sFilterConfig->FilterIndex, (hfdcan->Init.ExtFiltersNbr - 1U)));
			assert_param(
					IS_FDCAN_MAX_VALUE(sFilterConfig->FilterID1, 0x1FFFFFFFU));
			assert_param(
					IS_FDCAN_MAX_VALUE(sFilterConfig->FilterID2, 0x1FFFFFFFU));
			assert_param(IS_FDCAN_EXT_FILTER_TYPE(sFilterConfig->FilterType));

			/* Build first word of filter element */
			FilterElementW1 = ((sFilterConfig->FilterConfig << 29U)
					| sFilterConfig->FilterID1);

			/* Build second word of filter element */
			FilterElementW2 = ((sFilterConfig->FilterType << 30U)
					| sFilterConfig->FilterID2);
...
HAL_StatusTypeDef HAL_FDCAN_ConfigRxFifoOverwrite(FDCAN_HandleTypeDef *hfdcan,
		uint32_t RxFifo, uint32_t OperationMode) {
	/* Check function parameters */
...
HAL_StatusTypeDef HAL_FDCAN_ConfigTimeoutCounter(FDCAN_HandleTypeDef *hfdcan,
		uint32_t TimeoutOperation, uint32_t TimeoutPeriod) {
	/* Check function parameters */
...
HAL_StatusTypeDef HAL_FDCAN_GetRxMessage(FDCAN_HandleTypeDef *hfdcan,
	...
	if (state == HAL_FDCAN_STATE_BUSY) {
		...
		{
			pRxHeader->Identifier = ((*RxAddress & FDCAN_ELEMENT_MASK_STDID)
					>> 18U);
		...
		pRxHeader->FilterIndex =
				((*RxAddress & FDCAN_ELEMENT_MASK_FIDX) >> 24U);

		/* Retrieve NonMatchingFrame */
		pRxHeader->IsFilterMatchingFrame = ((*RxAddress
				& FDCAN_ELEMENT_MASK_ANMF) >> 31U);
...
HAL_StatusTypeDef HAL_FDCAN_GetTxEvent(FDCAN_HandleTypeDef *hfdcan,
	...
	if (state == HAL_FDCAN_STATE_BUSY) {
		...
		GetIndex = ((hfdcan->Instance->TXEFS & FDCAN_TXEFS_EFGI)
		...
		{
			pTxEvent->Identifier = ((*TxEventAddress & FDCAN_ELEMENT_MASK_STDID)
					>> 18U);
		} else /* Extended ID element */
		{
		...
		/* Retrieve ErrorStateIndicator */
		pTxEvent->ErrorStateIndicator = (*TxEventAddress
		...
		pTxEvent->MessageMarker = ((*TxEventAddress & FDCAN_ELEMENT_MASK_MM)
				>> 24U);
...
HAL_StatusTypeDef HAL_FDCAN_GetHighPriorityMessageStatus(
	...
	HpMsgStatus->FilterList = ((hfdcan->Instance->HPMS & FDCAN_HPMS_FLST)
			>> FDCAN_HPMS_FLST_Pos);
	HpMsgStatus->FilterIndex = ((hfdcan->Instance->HPMS & FDCAN_HPMS_FIDX)
			>> FDCAN_HPMS_FIDX_Pos);
...
HAL_StatusTypeDef HAL_FDCAN_GetProtocolStatus(const FDCAN_HandleTypeDef *hfdcan,
	...
	ProtocolStatus->DataLastErrorCode = ((StatusReg & FDCAN_PSR_DLEC)
			>> FDCAN_PSR_DLEC_Pos);
	ProtocolStatus->Activity = (StatusReg & FDCAN_PSR_ACT);
	ProtocolStatus->ErrorPassive = ((StatusReg & FDCAN_PSR_EP)
			>> FDCAN_PSR_EP_Pos);
	...
	ProtocolStatus->RxBRSflag = ((StatusReg & FDCAN_PSR_RBRS)
			>> FDCAN_PSR_RBRS_Pos);
	ProtocolStatus->RxFDFflag = ((StatusReg & FDCAN_PSR_REDL)
			>> FDCAN_PSR_REDL_Pos);
	ProtocolStatus->ProtocolException = ((StatusReg & FDCAN_PSR_PXE)
			>> FDCAN_PSR_PXE_Pos);
	ProtocolStatus->TDCvalue = ((StatusReg & FDCAN_PSR_TDCV)
			>> FDCAN_PSR_TDCV_Pos);
...
HAL_StatusTypeDef HAL_FDCAN_GetErrorCounters(const FDCAN_HandleTypeDef *hfdcan,
	...
	ErrorCounters->TxErrorCnt = ((CountersReg & FDCAN_ECR_TEC)
			>> FDCAN_ECR_TEC_Pos);
	ErrorCounters->RxErrorCnt = ((CountersReg & FDCAN_ECR_REC)
			>> FDCAN_ECR_REC_Pos);
	ErrorCounters->RxErrorPassive = ((CountersReg & FDCAN_ECR_RP)
			>> FDCAN_ECR_RP_Pos);
	ErrorCounters->ErrorLogging = ((CountersReg & FDCAN_ECR_CEL)
			>> FDCAN_ECR_CEL_Pos);
...
uint32_t HAL_FDCAN_IsRestrictedOperationMode(const FDCAN_HandleTypeDef *hfdcan) {
	...
	/* Get Operation Mode */
	OperationMode = ((hfdcan->Instance->CCCR & FDCAN_CCCR_ASM)
			>> FDCAN_CCCR_ASM_Pos);
...
HAL_StatusTypeDef HAL_FDCAN_ConfigInterruptLines(FDCAN_HandleTypeDef *hfdcan,
		uint32_t ITList, uint32_t InterruptLine) {
	HAL_FDCAN_StateTypeDef state = hfdcan->State;
...
HAL_StatusTypeDef HAL_FDCAN_ActivateNotification(FDCAN_HandleTypeDef *hfdcan,
	...
	if ((ActiveITs & (FDCAN_IT_TX_COMPLETE | FDCAN_IT_TX_ABORT_COMPLETE))
			!= 0U) {
	...
	if ((state == HAL_FDCAN_STATE_READY) || (state == HAL_FDCAN_STATE_BUSY)) {
		...
		if ((((ActiveITs & FDCAN_IT_LIST_RX_FIFO0) != 0U)
				...
				|| (((ActiveITs & FDCAN_IT_LIST_RX_FIFO1) != 0U)
						&& (((ITs_lines_selection) & FDCAN_IT_GROUP_RX_FIFO1)
								== 0U))
				|| (((ActiveITs & FDCAN_IT_LIST_SMSG) != 0U)
						&& (((ITs_lines_selection) & FDCAN_IT_GROUP_SMSG) == 0U))
				|| (((ActiveITs & FDCAN_IT_LIST_TX_FIFO_ERROR) != 0U)
						&& (((ITs_lines_selection)
								& FDCAN_IT_GROUP_TX_FIFO_ERROR) == 0U))
				|| (((ActiveITs & FDCAN_IT_LIST_MISC) != 0U)
						&& (((ITs_lines_selection) & FDCAN_IT_GROUP_MISC) == 0U))
				|| (((ActiveITs & FDCAN_IT_LIST_BIT_LINE_ERROR) != 0U)
						&& (((ITs_lines_selection)
								& FDCAN_IT_GROUP_BIT_LINE_ERROR) == 0U))
				|| (((ActiveITs & FDCAN_IT_LIST_PROTOCOL_ERROR) != 0U)
						&& (((ITs_lines_selection)
								& FDCAN_IT_GROUP_PROTOCOL_ERROR) == 0U))) {
...
void HAL_FDCAN_IRQHandler(FDCAN_HandleTypeDef *hfdcan) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TX_ABORT_COMPLETE) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(itsource, FDCAN_IT_TX_ABORT_COMPLETE)
				!= RESET) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_TIMEOUT_OCCURRED) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(itsource, FDCAN_IT_TIMEOUT_OCCURRED)
				!= RESET) {
	...
	if (FDCAN_CHECK_FLAG(itflag, FDCAN_FLAG_RAM_ACCESS_FAILURE) != RESET) {
		if (FDCAN_CHECK_IT_SOURCE(itsource, FDCAN_IT_RAM_ACCESS_FAILURE)
				!= RESET) {
...
static void FDCAN_CalcultateRamBlockAddresses(FDCAN_HandleTypeDef *hfdcan) {
	...
	for (RAMcounter = SramCanInstanceBase;
			RAMcounter < (SramCanInstanceBase + SRAMCAN_SIZE); RAMcounter +=
					4U) {
...
static void FDCAN_CopyMessageToRAM(const FDCAN_HandleTypeDef *hfdcan,
	...
	if (pTxHeader->IdType == FDCAN_STANDARD_ID) {
		...
		FDCAN_STANDARD_ID | pTxHeader->TxFrameType
	...
	TxAddress = (uint32_t*) (hfdcan->msgRam.TxFIFOQSA
			+ (BufferIndex * SRAMCAN_TFQ_SIZE));
